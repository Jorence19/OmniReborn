...
<head>
    ...
    <style>
        ...
        * {{
        ...
        .range-filter-val-badge.rug-badge {{
            color: #f59e0b;
...
<body>
        ...
        <div class="page-content active" id="page-leads">
            <div class="filter-bar">
                ...
                <div class="filter-sliders-row">
                    <!-- ATH Min/Max Range Slider Card -->
                    <div class="range-filter-card">
                        <div class="range-filter-header">
                            <span class="range-filter-title">💰 Peak ATH Range</span>
                            <span id="ath-range-badge" class="range-filter-val-badge">$0 — Max</span>
                        </div>
                        <div class="range-controls-grid">
                            <div class="range-control-item">
                                <span class="range-control-label">Min</span>
                                <input type="range" id="slider-ath-min" min="0" max="15" step="1" value="0" class="range-slider-input" oninput="onAthSliderChange('min')">
                                <span id="ath-min-pill" class="range-val-pill">$0</span>
                            </div>
                            <div class="range-control-item">
                                <span class="range-control-label">Max</span>
                                <input type="range" id="slider-ath-max" min="0" max="15" step="1" value="15" class="range-slider-input" oninput="onAthSliderChange('max')">
                                <span id="ath-max-pill" class="range-val-pill">Max</span>
                            </div>
                        </div>
                        <div class="range-footer-row">
                            <div class="range-preset-btns">
                                <span style="font-size: 10px; text-transform: uppercase; letter-spacing: 0.5px; margin-right: 2px;">Presets:</span>
                                <button type="button" class="preset-btn" onclick="setAthPreset(0, 15)">All</button>
                                <button type="button" class="preset-btn" onclick="setAthPreset(8, 15)">≥ $1M</button>
                                <button type="button" class="preset-btn" onclick="setAthPreset(5, 15)">≥ $100K</button>
                                <button type="button" class="preset-btn" onclick="setAthPreset(3, 7)">$25K - $500K</button>
                                <button type="button" class="preset-btn" onclick="setAthPreset(0, 2)">&lt; $10K</button>
                            </div>
                        </div>
                    </div>

                    <!-- Time to Rug Min/Max Range Slider Card -->
                    <div class="range-filter-card">
                        <div class="range-filter-header">
                            <span class="range-filter-title">⏱️ Time to Rug Range</span>
                            <span id="rug-range-badge" class="range-filter-val-badge rug-badge">0m — Max</span>
                        </div>
                        <div class="range-controls-grid">
                            <div class="range-control-item">
                                <span class="range-control-label">Min</span>
                                <input type="range" id="slider-rug-min" min="0" max="15" step="1" value="0" class="range-slider-input rug-slider" oninput="onRugSliderChange('min')">
                                <span id="rug-min-pill" class="range-val-pill">0m</span>
                            </div>
                            <div class="range-control-item">
                                <span class="range-control-label">Max</span>
                                <input type="range" id="slider-rug-max" min="0" max="15" step="1" value="15" class="range-slider-input rug-slider" oninput="onRugSliderChange('max')">
                                <span id="rug-max-pill" class="range-val-pill">Max</span>
                            </div>
                        </div>
                        <div class="range-footer-row">
                            <div class="range-preset-btns">
                                <span style="font-size: 10px; text-transform: uppercase; letter-spacing: 0.5px; margin-right: 2px;">Presets:</span>
                                <button type="button" class="preset-btn" onclick="setRugPreset(0, 15)">All</button>
                                <button type="button" class="preset-btn" onclick="setRugPreset(0, 3)">&lt; 5m Quick</button>
                                <button type="button" class="preset-btn" onclick="setRugPreset(3, 6)">5m - 25m</button>
                                <button type="button" class="preset-btn" onclick="setRugPreset(6, 15)">≥ 25m</button>
                                <button type="button" class="preset-btn" onclick="setRugPreset(8, 15)">≥ 1h</button>
                            </div>
                            <label class="alive-checkbox-label" title="Keep alive tokens visible when filtering rug times">
                                <input type="checkbox" id="rug-include-alive" onchange="filterLeadsTable()"> Include Still Alive
                            </label>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        const ATH_STEPS = [
            ...
            {{ val: 2500000, label: "$2.5M" }},
            {{ val: 5000000, label: "$5M" }},
            {{ val: 10000000, label: "$10M" }},
            {{ val: 25000000, label: "$25M" }},
            {{ val: 50000000, label: "$50M" }},
            {{ val: 100000000, label: "$100M" }},
            {{ val: Infinity, label: "Max" }}
        ];

        const RUG_TIME_STEPS = [
            {{ sec: 0, label: "0m" }},
            {{ sec: 60, label: "1m" }},
            {{ sec: 180, label: "3m" }},
            {{ sec: 300, label: "5m" }},
            ...
            {{ sec: 259200, label: "3d" }},
            {{ sec: 604800, label: "7d" }},
            {{ sec: Infinity, label: "Max" }}
        ];

        let activeAthMinStep = 0;
        let activeAthMaxStep = 15;
        let activeRugMinStep = 0;
        let activeRugMaxStep = 15;

        function onAthSliderChange(changed) {{
            const minEl = document.getElementById('slider-ath-min');
            const maxEl = document.getElementById('slider-ath-max');
            if (!minEl || !maxEl) return;

            let minIdx = parseInt(minEl.value, 10);
            let maxIdx = parseInt(maxEl.value, 10);

            if (changed === 'min' && minIdx > maxIdx) {{
                maxIdx = minIdx;
                maxEl.value = maxIdx;
            }} else if (changed === 'max' && maxIdx < minIdx) {{
                minIdx = maxIdx;
                minEl.value = minIdx;
            }}

            activeAthMinStep = minIdx;
            activeAthMaxStep = maxIdx;

            updateAthDisplay();
            renderLeadsTable();
        }}

        function setAthPreset(minIdx, maxIdx, triggerRender = true) {{
            const minEl = document.getElementById('slider-ath-min');
            const maxEl = document.getElementById('slider-ath-max');
            if (minEl) minEl.value = minIdx;
            if (maxEl) maxEl.value = maxIdx;
            activeAthMinStep = minIdx;
            activeAthMaxStep = maxIdx;
            updateAthDisplay();
            if (triggerRender) renderLeadsTable();
        }}

        function updateAthDisplay() {{
            const minLabel = (ATH_STEPS[activeAthMinStep] || ATH_STEPS[0]).label;
            const maxLabel = (ATH_STEPS[activeAthMaxStep] || ATH_STEPS[15]).label;
            const minPill = document.getElementById('ath-min-pill');
            const maxPill = document.getElementById('ath-max-pill');
            const rangeBadge = document.getElementById('ath-range-badge');

            if (minPill) minPill.textContent = minLabel;
            if (maxPill) maxPill.textContent = maxLabel;
            if (rangeBadge) {{
                rangeBadge.textContent = `${{minLabel}} — ${{maxLabel}}`;
            }}
        }}

        function onRugSliderChange(changed) {{
            const minEl = document.getElementById('slider-rug-min');
            const maxEl = document.getElementById('slider-rug-max');
            if (!minEl || !maxEl) return;
            ...
            activeRugMinStep = minIdx;
            activeRugMaxStep = maxIdx;

            updateRugDisplay();
            renderLeadsTable();
        }}

        function setRugPreset(minIdx, maxIdx, triggerRender = true) {{
            const minEl = document.getElementById('slider-rug-min');
            const maxEl = document.getElementById('slider-rug-max');
            if (minEl) minEl.value = minIdx;
            if (maxEl) maxEl.value = maxIdx;
        ...
        function updateRugDisplay() {{
            const minLabel = (RUG_TIME_STEPS[activeRugMinStep] || RUG_TIME_STEPS[0]).label;
            const maxLabel = (RUG_TIME_STEPS[activeRugMaxStep] || RUG_TIME_STEPS[15]).label;
            const minPill = document.getElementById('rug-min-pill');
            const maxPill = document.getElementById('rug-max-pill');
            const rangeBadge = document.getElementById('rug-range-badge');
        ...
        function renderLeadsTable() {{
            ...
            const minAth = ATH_STEPS[activeAthMinStep] ? ATH_STEPS[activeAthMinStep].val : 0;
            const maxAth = ATH_STEPS[activeAthMaxStep] ? ATH_STEPS[activeAthMaxStep].val : Infinity;
            const minRugSec = RUG_TIME_STEPS[activeRugMinStep] ? RUG_TIME_STEPS[activeRugMinStep].sec : 0;
            const maxRugSec = RUG_TIME_STEPS[activeRugMaxStep] ? RUG_TIME_STEPS[activeRugMaxStep].sec : Infinity;
            ...
            let filtered = currentCandidates.filter(c => {{
                ...
                if (candAth < minAth) return false;
                if (maxAth !== Infinity && candAth > maxAth) return false;
        ...
        function resetLeadsFilters() {{
            ...
            setAthPreset(0, 15, false);
            setRugPreset(0, 15, false);
