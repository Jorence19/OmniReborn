...
Production collection also requires a private/archive-capable Robinhood Chain RPC. Robinhood documents the public RPC as rate-limited and not recommended for production or historical indexing. `python streamer.py --preflight` deliberately fails while the public RPC is configured.
...
The on-chain backfill scans every Uniswap v4 `Initialize` event emitted by the official Robinhood Chain PoolManager. This is exhaustive for pools created through that PoolManager, not for contracts that never create a Uniswap v4 pool. DexScreener boosts, profiles, ads, and dated keyword searches supplement market and paid signals; search results alone are not represented as exhaustive.
...
- `ROBINHOOD_RPC_URL` pointing to an archive-capable Alchemy, Chainstack, QuickNode, or equivalent endpoint
