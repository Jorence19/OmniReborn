...
install -d -o "$APP_USER" -g "$APP_USER" -m 0750 "$WEB_DIR"
...
if [[ ! -f /etc/omnireborn.env ]]; then
  ...
  echo "Created /etc/omnireborn.env. Replace every placeholder, then rerun." >&2
  exit 2
fi
...
units=(
  ...
  omnireborn-api.service
  omnireborn-api-watchdog.service
  omnireborn-api-watchdog.timer
)
for unit in "${units[@]}"
do
...
sudo -u "$APP_USER" /bin/bash -c 'set -a; source /etc/omnireborn.env; set +a; exec /opt/omnireborn/.venv/bin/python /opt/omnireborn/phase1.py audit --db "$FORENSICS_DB_PATH" --output "$REPORT_OUTPUT_PATH"'
sudo -u "$APP_USER" /bin/bash -c 'set -a; source /etc/omnireborn.env; set +a; export DASHBOARD_OUTPUT_DIR=/opt/omnireborn/runtime/site-build; exec /opt/omnireborn/.venv/bin/python /opt/omnireborn/generate_html_dashboard.py'
...
sudo -u "$APP_USER" /bin/bash -c 'set -a; source /etc/omnireborn.env; set +a; exec /opt/omnireborn/.venv/bin/python /opt/omnireborn/api_server.py --check'
...
echo "Backend installed. Bind Nginx to the API domain, obtain TLS, then run the smoke tests."
